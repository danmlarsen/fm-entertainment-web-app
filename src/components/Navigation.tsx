...
export default function Navigation() {
  ...
  return (
    <nav className="flex gap-6 md:gap-8 lg:flex-col lg:gap-10">
      {links.map((link) => (
        <Link
          className={`flex items-center justify-center transition duration-300 hover:text-primary-500 focus-visible:text-primary-500 focus-visible:outline-none ${pathname === link.href ? "text-white" : "text-secondary-500"}`}
          key={link.title}
          href={link.href}
        >
          {link.icon}
